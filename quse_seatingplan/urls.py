...
def _namespaced(pattern, view, name):
    ...
    widget = event_url(r'w/(?P<cart_namespace>[a-zA-Z0-9]{16})/' + tail, view, name=name)
...
for route in (
    ...
    (r'^(?P<subevent>[0-9]+)/quse-seatingplan/frame/$', EmbeddedSeatingPlanView.as_view(), 'frame'),
    (r'^quse-seatingplan/api/data/$', SeatingPlanDataView.as_view(), 'seat-data'),
    (r'^(?P<subevent>[0-9]+)/quse-seatingplan/api/data/$', SeatingPlanDataView.as_view(), 'seat-data'),
    (r'^quse-seatingplan/api/assign/$', SeatAssignmentView.as_view(), 'seat-assign'),
    (r'^(?P<subevent>[0-9]+)/quse-seatingplan/api/assign/$', SeatAssignmentView.as_view(), 'seat-assign'),
):
