...
class SeatingPlanSettingsForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.is_bound:
            ...
            self.initial['plan_name'] = stored_name or (
                self.event.seating_plan.name if self.event.seating_plan else self.event.name
            )
    ...
    def _add_category_fields(self, categories: List[str]) -> None:
        mapping_rows = SeatCategoryMapping.objects.filter(event=self.event, subevent=None)
        ...
        for category in categories:
            ...
            self.fields[field_name] = forms.ModelMultipleChoiceField(
                ...
                queryset=item_qs,
                required=False,
                help_text=_('Select one or more products that may sell seats in this category.'),
            )
    ...
    def clean_plan_file(self) -> Optional[str]:
        ...
        if not self._plan_data or not self._plan_document:
            ...
            try:
                ...
                raise forms.ValidationError(_('Could not decode JSON: %(error)s'), params={'error': exc})
            self._plan_document = decoded
            self._plan_data = data
        try:
    ...
    def clean(self):
        cleaned = super().clean()
        if not self.event.seating_plan and not (self._plan_data or cleaned.get('plan_file')):
    ...
    def _persist_category_mapping(self) -> dict:
        ...
        for category in self._category_names:
            seen.add(category)
            products = list(self.cleaned_data.get(self._category_field_name(category)) or [])
        ...
        SeatCategoryMapping.objects.filter(event=self.event, subevent=None).exclude(layout_category__in=seen).delete()
        return mapping
