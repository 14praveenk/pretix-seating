...
from pretix.base.models import CartPosition, Event, Item, Organizer, Order
...
def test_settings_form_disables_default_seat_choice(monkeypatch):
    ...
    monkeypatch.setattr('quse_seatingplan.forms.seating_service.generate_seats', lambda *a, **k: None)
...
def test_embedded_view_sets_xframe(monkeypatch):
    ...
    monkeypatch.setattr('quse_seatingplan.views.SeatingPlanView.dispatch', fake_dispatch)
...
def _pretix_request(event, method='get', path='/', data=None, content_type='application/json'):
    rf = RequestFactory()
...
def test_seating_data_view_returns_payload():
    ...
    with scope(organizer=organizer):
        ...
        SeatCategoryMapping.objects.create(event=event, subevent=None, layout_category='Front', product=item)
        event.seats.create(seat_guid='Front-A-1', zone_name='Front', row_name='A', seat_number='1', product=item, x=10, y=20)
    ...
    rectangle = next(shape for shape in payload['shapes'] if shape['type'] == 'rectangle')
...
def test_seat_assignment_view_assigns_and_clears():
    ...
    with scope(organizer=organizer):
        item = event.items.create(name='Seat', default_price=10, admission=True)
        SeatCategoryMapping.objects.create(event=event, subevent=None, layout_category='Front', product=item)
        seat = event.seats.create(seat_guid='Front-A-1', zone_name='Front', row_name='A', seat_number='1', product=item, x=10, y=20)
...
def test_settings_view_plan_summary():
    ...
    with scope(organizer=organizer):
        item = event.items.create(name='Seat', default_price=10, admission=True)
        event.seats.create(seat_guid='VIP-1', seat_number='1', zone_name='VIP', product=item)
        SeatCategoryMapping.objects.create(event=event, subevent=None, layout_category='VIP', product=item)
...
def test_settings_form_supports_multiple_products(monkeypatch):
    ...
    monkeypatch.setattr('quse_seatingplan.forms.seating_service.generate_seats', fake_generate)
    ...
    mappings = SeatCategoryMapping.objects.filter(event=event, layout_category='Front').values_list('product_id', flat=True)
...
def test_checkout_step_not_applicable_without_matching_items():
    ...
    with scope(organizer=organizer):
        ...
        SeatCategoryMapping.objects.create(event=event, subevent=None, layout_category='A', product=seat_item)

    request = _request_for_event(event)
    positions = [SimpleNamespace(item_id=other_item.pk, seat=None, subevent=None, subevent_id=None)]
...
def test_checkout_step_requires_filled_seats():
    ...
    request = _request_for_event(event)
    positions = [SimpleNamespace(item_id=seat_item.pk, seat=None, subevent=None, subevent_id=None)]
...
def test_perform_order_preserves_plugin_seats():
    ...
    with scope(organizer=organizer):
        item = event.items.create(name='Seat', default_price=Decimal('10.00'), admission=True)
        SeatCategoryMapping.objects.create(event=event, subevent=None, layout_category='Main', product=item)
